...
model, `T = 2(N-1)α + 2·(N-1)/N·bytes·β`, for plausible-*order-of-magnitude*
`(α, β)` per algorithm, then draws 20 lognormal-perturbed samples around that
mean per configuration (matching `apps/benchmark.cpp`'s CSV schema exactly:
one row per `{algorithm, num_procs, dtype, message_size_bytes, time_stat}`).
The `(α, β)` constants are not fitted to, derived from, or claimed to
represent any real machine — including the i7-14700K / 32GB DDR5 workstation
this project's build spec names as the intended target hardware.
...
at the real CSVs and every figure, table, and fitted α/β in `report/main.pdf`
will regenerate from genuine measurements.
