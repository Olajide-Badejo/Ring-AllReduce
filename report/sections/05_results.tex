...
Figure~\ref{fig:latency-decomposition} isolates the smallest swept message size and, for the custom ring specifically, breaks the cost model's prediction into its two additive terms per process count, alongside a raw \texttt{MPI\_Send}/\texttt{MPI\_Recv} ping-pong one-way latency measured on the same hardware in the same run (\texttt{apps/pingpong}, using the same one-way-latency-from-round-trip convention as OSU Micro-Benchmarks' \texttt{osu\_latency} \citep{osu2024benchmarks}). At this message size the bandwidth term is negligible at every $N$, confirming that the modeled latency term $2(N-1)\alpha$ is essentially the entire predicted cost once messages are small enough. The measured ping-pong one-way latency is \textbf{0.1585~$\mu$s}, against the custom ring's fitted per-step $\alpha$ of \textbf{0.264~$\mu$s}; Section~\ref{sec:discussion} uses that gap to quantify, rather than assert, how much of the ring's small-message cost is genuine per-message latency and how much is rank-to-rank synchronization overhead.
